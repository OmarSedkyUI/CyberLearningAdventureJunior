...
public class Level2_Friend_Level2 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (GameObject.Find("Hacker").GetComponent<Level2_Hacker_Level2>().FriendAppear && oneTime)
        {
        ...
        if (IsGrounded())
        {
            Choice = GameObject.Find("Hacker").GetComponent<Level2_Hacker_Level2>().choice;
            if (Choice == 1 && Repeat)
            {
                index = 3;
                int LinesNo = 2;
        ...
        if(GameObject.Find("Hacker").GetComponent<Level2_Hacker_Level2>().passwrodStolen)
        {
            if (Vector2.Distance(player.position, transform.position) < 2.5f)
            {
                if (!dialogue.activeSelf)
